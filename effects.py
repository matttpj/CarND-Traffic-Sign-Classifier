...
#        return shift_img
    ...
    def image_rotate(self, img):
        row, col, channel = img.shape # for color images
    ...
    def image_shear(self, img):
        x, y, channel = img.shape  # for color images
    ...
    def image_gray(self, img):
        gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        #dim = np.zeros_like(gray)
        new_img = np.stack((gray, gray, gray), axis=2)
        return new_img
